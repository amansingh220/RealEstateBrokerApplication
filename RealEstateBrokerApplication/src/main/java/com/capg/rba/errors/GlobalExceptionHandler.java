...
public class GlobalExceptionHandler {
		...
		/*
		 * // Handles user-defined exception that is thrown when we try to update // the
		 * customer by passing an custId that does not exist in database table.
		 * 
		 * @ExceptionHandler(InvalidCustIdException.class) private
		 * ResponseEntity<String> handleInvalidCustIdException(InvalidCustIdException
		 * exception) { ResponseEntity<String> responseEntity = new
		 * ResponseEntity<String>(exception.getMessage(), HttpStatus.BAD_REQUEST);
		 * return responseEntity; }
		 */
